...
public final class XingsingActionMaskBuilder {
    public boolean[] build(Xingsing mob, XingsingObservation obs) {
        ...
        allow(mask, XingsingOption.KEEP_PLAY_DISTANCE, obs.nearestPlayerVisible && obs.nearestPlayerDistanceNorm < 0.28F);
        ...
        allow(mask, XingsingOption.MIRROR_SPRINT, obs.nearestPlayerSprinting && obs.dashPathScore > 0.45F);
        ...
        allow(mask, XingsingOption.PLAY_CHASE, obs.trust > 0.35F && obs.playInterestScore > 0.35F
                && obs.dashPathScore > 0.45F);
        allow(mask, XingsingOption.CLIMB_TO_PERCH, obs.perchReachable && obs.perchEscapeScore > 0.25F);
        allow(mask, XingsingOption.WARN_HOSTILE, obs.nearestHostileUuid != null && obs.threatScore > 0.25F);
        allow(mask, XingsingOption.FLEE_TO_TREE, obs.escapePathScore > 0.25F
                && (obs.threatScore > 0.4F || obs.fear > 0.65F || obs.healthRatio < 0.35F));
